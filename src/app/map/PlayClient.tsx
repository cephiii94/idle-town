...
import GameCanvas, { GameCanvasHandle, ResourceType, TownData, Villager } from '@/components/GameCanvas';
...
export default function PlayClient({ selectedStage }: PlayClientProps) {
  ...
  const [townData, setTownData] = useState<TownData>({ houseLevel: 1, capacity: 3, villagerCount: 2 });
  ...
  const handleGainResource = useCallback((item: ResourceType, amount: number) => {
    setResources((prev) => ({
      ...prev,
      ...
      <header className="w-full sticky top-0 z-40 bg-slate-900/80 backdrop-blur-md border-b border-white/5 px-4 py-2 flex justify-center flex-none">
        <div className="w-full max-w-7xl flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <Link
              href="/stage"
              className="hidden sm:flex items-center gap-2 bg-slate-950/60 hover:bg-slate-800 border border-white/10 px-3 py-2 rounded-xl transition-colors"
            >
              <span className="text-base">{selectedStage.emoji}</span>
              <span className="flex flex-col leading-none">
                <span className="text-[8px] font-black text-slate-500 uppercase tracking-[0.18em] mb-1">Stage</span>
                <span className="text-[10px] font-bold text-slate-200">{selectedStage.name}</span>
              </span>
            </Link>
